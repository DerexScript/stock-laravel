...
@section('content')
    ...
    <table class="table table-striped table-hover mt-1 mb-5">
        ...
        @foreach($categories as $category)
            <form id="form_delete_{{$loop->iteration}}" action="{{route('category.destroy',$category->id)}}" method="POST">
                @method('DELETE')
                @csrf
            </form>
            <form id="form_edit_{{$loop->iteration}}" action="{{route('category.edit', $category->id)}}" method="GET"></form>
            <tr>
